...
const sharp=require(process.env.AVFX_SHARP || 'sharp');
import { writeFile } from 'node:fs/promises';
import { resolve, join } from 'node:path';
const [bundle, captures, engine] = process.argv.slice(2);
if (!bundle || !captures || !['godot','unity','unreal'].includes(engine)) throw new Error('Expected bundle, capture directory and godot|unity|unreal');
const rows=[];
