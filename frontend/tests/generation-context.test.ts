...
import {
  ...
  reviewGeneration,
  modelStage,
} from "../src/lib/studio-tools/legacy-generation";
...
test("both generation calls receive the same server-resolved brief and image evidence", async () => {
  ...
  const wire = {
    ...
    layers: doc.layers.map((l) => ({
      ...
      light: l.light ?? null,
      blob: l.blob ?? null,
      splash: l.splash ?? null,
      ribbon: l.ribbon ?? null,
      wireBurst: l.wireBurst ?? null,
      crystals: l.crystals ?? null,
      arcs: l.arcs ?? null,
      streakBurst: l.streakBurst ?? null,
      reflection: l.reflection ?? null,
      sheets: l.sheets ?? null,
      crescent: l.crescent ?? null,
      licks: l.licks ?? null,
    })),
  };
...
import { repairGeneration } from "../src/lib/studio-tools/legacy-generation";
