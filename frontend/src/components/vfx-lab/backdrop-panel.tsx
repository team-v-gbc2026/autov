...
export function BackdropPanel({
  ...
  // Seed from the remembered backdrop so a restored workspace offers one-click
  // Load; the asset is no longer fetched at startup.
  const [url, setUrl] = useState(snapshot.settings.url ?? presets[0]?.url ?? "");
  const fileRef = useRef<HTMLInputElement | null>(null);
  const { state, settings, error, numSplats } = snapshot;
  const busy = state === "loading";
  /** Remembered but not fetched: a stored URL with nothing on screen. */
  const remembered = state === "empty" && !!settings.url;
  ...
  const hint = {
    // "empty" now covers two cases: nothing set, and set-but-not-yet-fetched.
    empty: remembered ? "remembered — press Load" : "no backdrop",
    loading: "loading…",
  ...
  return (
    <div style={panelStyle} data-testid="backdrop-panel">
      ...
      <input
        ...
        type="text"
        value={url}
        aria-label="Gaussian splat PLY URL"
        placeholder="/path/to/splat.ply"
        onChange={(e) => setUrl(e.target.value)}
        disabled={!controller || busy}
      ...
      <input
        ...
        type="file"
        accept=".ply"
        aria-label="Import Gaussian splat PLY file"
        style={{ display: "none" }}
        onChange={(e) => {
