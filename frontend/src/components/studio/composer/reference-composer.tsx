...
export default function ReferenceComposer({ ref, references, busy, saving, uploadFile, onSend }: {
  ...
  return <ReferenceContext.Provider value={{ references, retry: id => { void retry(id); } }}>
    <form className={`composer ${styles.composer}`} onSubmit={event => { event.preventDefault(); void send(); }}>
      ...
      </div><button type="submit" className="send-button" disabled={!hasText || saving || busy || uploading} aria-label="Save prompt"><Icon name="arrow" /></button></div>
