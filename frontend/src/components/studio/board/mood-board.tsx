...
export default function MoodBoard({ projectId, state, onMention, onCollapse, locked, focusRequest }: {
  ...
  return <aside className={`glass ${styles.panel} ${expanded ? styles.expanded : ""}`}>
    ...
    {preview && <ReferencePreview key={`${preview.id}-${regenerating ? "regenerate" : "preview"}`} reference={preview} promptInitiallyOpen={regenerating} onEdit={state.editImage ? prompt => state.editImage!(preview.id, prompt) : undefined} disabled={state.busy || locked} onClose={() => { setPreviewId(null); setRegenerating(false); }} onRename={name => {
