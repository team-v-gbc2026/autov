...
export function browserExpiry(operation: {
  ...
  if (!operation.lease_id) return {
    ...
    message: capture
      ? "No Studio tab picked up the capture before its deadline. Keep this project's Studio open and recover the saved candidate; do not regenerate."
  ...
  if (capture && elapsed >= 600_000) return {
    ...
    message: "Capture reached its ten-minute limit. The saved candidate is preserved; recover it without regenerating.",
  };
  return {
    reason: "worker_lease_expired",
    message: capture
      ? "The Studio capture stopped renewing its lease. Keep the tab active and recover the saved candidate without regenerating."
