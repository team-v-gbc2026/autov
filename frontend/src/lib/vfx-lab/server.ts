...
Coordinate frame: author every effect in its own local frame — origin (0,0,0), up +Y, forward +Z, distances in meters, angles in radians. Layer positions, emission directions, motion paths and forces are all effect-local and must never assume a world location: the effect is placed into a scene separately by the editor, and that placement is not part of this document. Build the effect as if it happens at the origin, facing +Z. Semantic origin means the nozzle for a fountain, detonation point for an explosion, or launch point for a projectile, never the visual bounds center. Keep the main emission point at zero and never bake viewer placement into layer coordinates.
...
Additional per-layer fields (null uses legacy behavior): geometry=auto|plane|teardrop|cone|crystal|crystal-cluster|torus|ribbon|streamer|lightning; surface=default|flame|water|hexagon|smoke|star|solid|portal|water-streaks|energy-ribbon|circle-eyes|ice|sparkle; motion={keys:[[localSeconds,x,y,z],...],ease:linear|smooth}; textureId=provided asset ID or null.
Motion keys are LOCAL seconds and OFFSET params.position in meters, linearly interpolated or smoothstep. For curved trajectories use several waypoints. Directional surface meshes can move; particles move their entire emitter as a group, NOT a world-space trail. Use separate trailing layers when required. Never use motion on a beam when the requirement is a stationary single strike. Geometry plane is an upright XY rectangle, width=2*radius, height=length; for kind=sprite it faces the camera and rotation.z rotates it in that plane. surface portal draws its rectangular rim and mist; params.width is the rim thickness in meters (usually .05-.12). Beam auto is camera-facing vertical; rotation.z rotates it in the camera plane (pi/2 for horizontal). Ribbon and torus mesh spin rotates actual geometry deterministically. Geometry lightning is a seeded sharp branched 3D tube along local Y; params.length controls total height, params.width controls cross-section radius in meters (diameter approximately twice width), radius controls lateral zigzag reach. Co-located core and sheath share a stable centerline. turbulence < .3 disables side branches for graphic bolts. Put center at groundY+length/2 so it hits the ground. Geometry crystal-cluster creates 1-32 deterministic faceted crystals in ONE surface layer, using count (usually 12-20), radius=ground footprint radius, width=individual crystal base radius (usually .15-.3), length=tallest crystal height. Its origin is the ground/root; animate length to grow upward without sinking the bases. Crystals lean outward with seeded height variation. Surface ice on a crystal or crystal-cluster provides colored faceted bodies and fine white fracture contours: use near-white color, blue secondaryColor, normal blend, intensity around1. This is useful for an area of ice; separate thin single crystals alone leave the attack area empty. Count is fixed at construction; width, radius and length can be animated. Teardrop is a rounded streamline mesh for a projectile head. Cone, teardrop and crystal extend along local Y; radius controls width, length controls height. Streamer is an open curved membrane along local Y (root at -length/2, tip at +length/2), scaled by radius in width and length in height; turbulence animates ONLY the free tip with a deterministic wave, preserving the attached root. Rotate Z=-pi/2 for a rightward tail, and overlap its root with the head. Torus and ribbon lie in XY; ribbon is an ARC strip controlled by radius/arc/width, not a straight tail: length has no effect on it. rotate to ground XZ when appropriate. Surface water-streaks draws ONLY moving highlight bands with transparent gaps; use it for optional white streaks on a slightly enlarged copy of a water head, matching center/rotation/motion/turbulence. Never use a second full water/solid shell for highlights, because it hides the colored body. Surface energy-ribbon on an auto beam provides a continuous sharp core in color and saturated side ribbons in secondaryColor, with a narrow animated edge controlled by turbulence; use color=#FFFFFF and secondaryColor=the desired beam hue. It maintains a solid interior along the whole length, so a single layer can supply both core and edge. Surface solid fills mesh; flame gives directional erosion, water streaks, hexagon shield cells, smoke soft low-frequency density, star a crisp five-point particle-like sprite. For smoke use normal blend, subdued color, low intensity, broad forms. For lightning use one primary mesh, optionally one narrower core, not hundreds of radial sparks. Enforce requested number of strikes and timing. For a projectile pointing left use a cone rotated Z=+pi/2 (tip points left). Texture on decal replaces the engraved pattern; other supported surfaces multiply silhouette by texture luminance and alpha. Texture masks must be white/grayscale; params colors perform tinting. For simple comic faces prefer one camera-facing sprite with surface circle-eyes: it draws a circular outline in color and two large eyes in secondaryColor with dark pupils; the eyes stay attached automatically. Use pink color and near-white secondaryColor, no texture. Alternatively use kind ring with separate sprite eyes grouped by matching motion keys. Keep faces clearly larger than their eyes and keep eye offsets fixed relative to each face. Surface star provides a straight-edged five-point silhouette on sprites and particles; surface sparkle provides a thin FOUR-point glint in one sprite/particle, avoiding two separately moving cross bars. Geometry plane with surface default preserves a sprite's soft glow; use it for tall translucent auras. Particles do not support generated texture sampling; never assign textureId to particles.
Layer start/end are GLOBAL seconds. Track keys are LOCAL seconds since layer.start and must fit end-start, strictly increasing. Numeric track values must stay in their target ranges: radius .01..8, width .001..3, length .01..12, intensity 0..8, opacity 0..1, speed -8..8, turbulence 0..2, erosion 0..1, spin -10..10. To disappear use opacity=0, never radius/width/length=0. Overrides must be [] for new generation. Params all required: irrelevant fields keep sensible defaults. count affects particles and crystal-cluster; other geometries ignore it. For particles, radius is birth radius, emission is birth interval (hashed IDs), life is lifetime, speed radial velocity, spread controls vertical direction (0 planar XZ, 1 sphere), gravity is Y acceleration, drag damps velocity. Keep emission+life <= end-start to avoid clipping. No orbit/curl/physics simulation: do not claim unsupported motion. spin is surface texture rotation only.
