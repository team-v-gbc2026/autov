...
uniform vec4 uSmokeLightPosition[4],uSmokeLightColor[4];
...
vec3 smokeLighting(vec3 normal, vec3 world){
  ...
  for(int i=0;i<4;i++){
    vec3 delta=uSmokeLightPosition[i].xyz-world;
    float d=length(delta);
    vec3 direction=delta/max(d,.001);
    float wrap=clamp((dot(normal,direction)+.5)/1.5,0.,1.);
    float rangeFade=1.-smoothstep(uSmokeLightPosition[i].w*.75,uSmokeLightPosition[i].w,d);
    float attenuation=rangeFade/max(pow(max(d,.25),uSmokeLightColor[i].w),1.);
    light+=uSmokeLightColor[i].rgb*wrap*attenuation;
  }
  return light;
}
`;
