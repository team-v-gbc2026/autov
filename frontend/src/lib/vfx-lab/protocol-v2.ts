...
Layer: id (lowercase-dashes), name, role anticipation|primary|impact|secondary|residue, kind ring|shell|trail|beam|sprite|particles|decal|light|blob|splash|ribbon|wireBurst|crystals|arcs|streakBurst|reflection, start/end in GLOBAL seconds, enabled, transform{position,rotation,scale}, motion{keys:[[localSeconds,dx,dy,dz],...], ease}|null, jitter{frequency .5-60, amplitude 0-2, gate 0-1, axis (unit vector)|null}|null, collapse{start (layer-local seconds), duration, heightCurve, widthCurve, anchor:"base"}|null, window{at:{pathId,u}}|null, tracks (dotted-path keyframes, local seconds), overrides (must be [] for new generation).
...
emitter{count, shape{type point|sphere|hemisphere|cone|ring|disc|box|line|path|pathLine|frame|orbit, axis (unit vector), length, radius, innerRadius, angle, size, surfaceOnly, bias (0-1 per axis, mirrors spawns toward +axis), pathId|null, interiorFraction 0-1}, spawn{mode burst|continuous|bursts|pathAnchored|event, window, rate, duration, bursts[{t,count}], headCurve|null, originsFromPath}, velocity{mode radial|directional|tangential|cone|alongPath|orbit, speed [min,max], direction (unit vector), angle, inherit, speedCurve|null}, life [min,max], forces{gravity, drag 0-6, curl{strength,frequency,speed,envelope}|null, vortex{axis,strength,falloff}|null, wind, floor{y,softness}|null}, render{mode billboard|velocityStretch|horizontal|vertical|pathAligned|flatStrip, anchor center|head, stretch, size [min,max], sizeCurve, alphaCurve, alphaAlongSpawn|null, rotation{initial [min,max], speed [min,max]}, sortMode, twinkle{frequency .1-40, depth 0-1}|null, strip{length [min,max], width [min,max], waviness 0-1, stepRate 0-60, palettes 1|2}|null}, trail|null, sub|null}.
