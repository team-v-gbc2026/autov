...
Layer: id (lowercase-dashes), name, role anticipation|primary|impact|secondary|residue, kind ring|shell|trail|beam|sprite|particles|decal|light|blob|splash|ribbon|wireBurst|crystals, start/end in GLOBAL seconds, enabled, transform{position,rotation,scale}, motion{keys:[[localSeconds,dx,dy,dz],...], ease}|null, jitter{frequency .5-60, amplitude 0-2, gate 0-1, axis (unit vector)|null}|null, tracks (dotted-path keyframes, local seconds), overrides (must be [] for new generation).
...
- particles have no geometry: use emitter.shape; blob, splash, ribbon, wireBurst and crystals have none either (use blob.spread/height, splash.length, the ribbon's path, wireBurst.radius/travel and crystals.length/baseRadius).
