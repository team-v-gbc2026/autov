...
export function smokeLightingUniforms(
  ...
  return {
    ...
    // One buffer: mesh materials already use eleven other fragment buffers.
    // Positions occupy 0..3 and colors/decay 4..7.
    uSmokeLights: {
      value: Array.from(
        { length: SMOKE_LIGHT_LIMIT * 2 },
        () => new THREE.Vector4(),
      ),
    },
  };
...
export function updateSmokeLighting(
  ...
  scene.traverse((object) => {
    ...
    for (let i = 0; i < SMOKE_LIGHT_LIMIT; i++) {
      const position = uniforms.uSmokeLights.value[i] as THREE.Vector4;
      const color = uniforms.uSmokeLights.value[i + SMOKE_LIGHT_LIMIT] as THREE.Vector4;
