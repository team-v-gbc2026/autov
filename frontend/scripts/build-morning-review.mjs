...
for (const id of cases) {
  ...
  const versions = trials
    .map(
      (t) =>
        `<tr><td>${esc(t.name)}<br><small>${t.origin === "refined" ? "修正案" : "生成案"}${t.selected ? " · 採用" : ""}</small></td><td>${weighted(t.review)}</td><td>${t.video ? `<a href="${link(t, "video.webm")}">動画</a> · ` : ""}${t.player ? `<a href="${link(t, "player.html")}">3D再生</a> · ` : ""}<a href="${link(t, "document.json")}">JSON</a></td></tr>`,
    )
    .join("");
  const comparison = `<div class="compare"><section><h2>生成結果 · 最新の採用案</h2>${chosen.video ? `<video controls loop preload="metadata" src="${link(chosen, "video.webm")}"></video>` : `<img style="width:100%" src="${link(chosen, "sheet.jpg")}">`}<p>${chosen.player ? `<a class="pill" href="${link(chosen, "player.html")}">インタラクティブに再生・スクラブ ↗</a>` : ""}<a class="pill" href="${link(chosen, "document.json")}">編集用JSON</a></p></section><section><h2>元のエフェクト</h2>${chosen.referenceVideo ? `<video controls loop preload="metadata" src="${link(chosen, "reference.mp4")}"></video><p class="muted">元動画と生成側では尺・時刻が異なる場合があります。生成側のタイミングは入力プロンプトに従います。</p>` : `<p class="muted">このケースの元動画はアーカイブにありません。下の入力画像と比較してください。</p>`}</section></div>`;
