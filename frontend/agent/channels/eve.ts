...
import { ChatError, parseTurn, readBody, responseError } from "../lib/contracts";
  ...
  return { ...route, handler: async (request, args) => {
    ...
    try {
      ...
      if (isCancel) {
        ...
        return await route.handler(new Request(request, { body: JSON.stringify({ turnId: body.turnId }) }), args);
      }
      ...
      if (isCreate) {
        ...
        response = await route.handler(new Request(request, { body: JSON.stringify({ message, clientContext, turnPolicy: "queue" }) }), args);
      }
      return response;
    } catch (error) {
      console.warn("studio.agent.request", { code: error instanceof ChatError ? error.code : "INTERNAL_ERROR" });
