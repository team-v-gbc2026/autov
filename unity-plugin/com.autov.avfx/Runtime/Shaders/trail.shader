...
float selfBirth(float4 s, float t, float4 ev, float idx)
{
    ...
    if (uSpawnMode == 1)
    {
        ...
        bool _1454 = absBirth < 0.0f;
        bool _1463;
        if (!_1454)
        {
            _1463 = absBirth > uSpawnDuration;
        }
        else
        {
            _1463 = _1454;
...
void selfTraj(float4 s, float4 e, float4 e2, float3 srcPos, float3 srcDir, float4 ev, float idx, float age, float life, float t, inout float3 pos, inout float3 vel)
{
    ...
    if (uVelMode == 4)
    {
        ...
        float param_11 = lu;
        float _2533 = selfSpeedAt(param_11);
        float ru = clamp(_2533 - lag, 0.0f, 1.0f);
        float param_12 = ru;
    ...
    {
        float param_18 = u;
        float _2699 = selfSpeedI(param_18);
        d = life * _2699;
        float param_19 = u;
        float _2704 = selfSpeedAt(param_19);
        scale = _2704;
    }
    else
    {
    ...
    if (uPlanarDrag > 0.0f)
    {
        ...
        float2 _2816 = (((dir * v0) * sp) + uWind).xz;
        vel = float3(_2816.x, vel.y, _2816.y);
    }
    ...
    if (uCurl > 0.0f)
    {
        ...
        float param_23 = u;
        float _2924 = curveE(param_23);
        pos += (((c_2 * uCurl) * _2924) * a);
    }
    if (uHasFloor == 1)
    {
...
void vert_main()
{
    ...
    {
        _3240 = float2(sv.y, -sv.x) / svl.xx;
    }
    else
    {
        _3240 = float2(1.0f, 0.0f);
    }
    float2 across = _3240;
    float param_17 = u;
    float _3264 = curveA(param_17);
    float param_18 = kn;
    float _3268 = curveG(param_18);
    float width = ((lerp(uSize.x, uSize.y, aExtra2.x) * _3264) * _3268) * 0.5f;
    float2 _3278 = mv.xy + ((across * side) * width);
    mv = float4(_3278.x, _3278.y, mv.z, mv.w);
    gl_Position = mul(mv, projectionMatrix);
    float param_19 = u;
    float _3288 = curveB(param_19);
    vAlpha = _3288 * smoothstep(0.0f, 0.02999999932944774627685546875f, age);
    if (uHasAlphaSpawn == 1)
    {
        float param_20 = aExtra.w;
        float _3301 = curveD(param_20);
        vAlpha *= _3301;
    }
