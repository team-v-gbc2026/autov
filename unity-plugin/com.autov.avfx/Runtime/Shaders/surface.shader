...
#pragma fragment avfxFragment
    ...
    uniform float2 uZRange;
    uniform float3 uVertexBias;
    uniform float4 uCurveF[8];
    ...
    uniform int uBolt;
    uniform float2 uNoiseScale;
    uniform float2 uNoisePan;
    uniform float2 uMaskScale;
    uniform float2 uMaskPan;
    uniform float2 uDistortPan;
    uniform float3 uEdgeCol;
    uniform float3 uCam;
    ...
    uniform float3 uTileCol;
    uniform float3 uLatEdgeCol;
    uniform float3 uPlaneCol;
    uniform float4 uRipple[8];
    uniform int uSlabN;
    uniform float4 uSlabTier[8];
    uniform float uFlicker;
    uniform int uReflect;
    uniform float3 uReflectTint;
    ...
    uniform float4 uStreakA;
    uniform float4 uStreakB;
    uniform float3 uStreakCol;
    uniform float3 uCrease;
    ...
    uniform int uRampN;
    uniform float4 uCurveC[8];
    uniform int uCurveCN;
    uniform float uCurveCEase;
    uniform float4 uProcParams;
    uniform float4 uStripeA[3];
    uniform float4 uStripeB[3];
    uniform int uStripeN;
    uniform float3 uSymFill;
    uniform float3 uSymOutline;
    uniform float3 uSymHigh;
    uniform float3 uSymInk;
    uniform float3 uSymHot;
    uniform float uSymHotI;
    uniform float uSymHotA;
    uniform float uScreenPitch;
    uniform float uScreenOn;
    uniform float uScreenWorld;
    uniform float3 uScreenCol;
    uniform int uFrame;
    uniform int uSdfN;
    uniform int uBeadOn;
    ...
    uniform float4 uSdfHalo[3];
    uniform float3 uBeads;
    uniform int uFlowOn;
    uniform int uFlowN;
    uniform float4 uFlowLayer[8];
    uniform float3 uFlowCut;
    uniform int uSwirlOn;
    uniform float4 uSwirlBands;
    uniform float4 uSwirlDetail;
    uniform float3 uSwirlLobe;
    ...
    uniform float3 uSmokeRight;
    uniform float3 uSmokeUp;
    uniform float3 uSmokeForward;
    uniform float4 uSmokeLights[8];
...
void vert_main()
{
    ...
    if (uFrameV == 1)
    {
        float2 avfxHalfExtent = float2(uRadius + uFrameMargin, (uLength * 0.5f) + uFrameMargin);
        float3 local = float3(position.xy * avfxHalfExtent, 0.0f);
        vObj = local;
