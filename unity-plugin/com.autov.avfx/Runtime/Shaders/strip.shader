...
float selfBirth(float4 s, float t, float4 ev, float idx)
{
    ...
    if (uSpawnMode == 1)
    {
        ...
        bool _1385 = absBirth < 0.0f;
        bool _1394;
        if (!_1385)
        {
            _1394 = absBirth > uSpawnDuration;
        }
        else
        {
            _1394 = _1385;
...
float3 orthoOf(float3 a)
{
    ...
    {
        _643 = cross(a, float3(0.0f, 1.0f, 0.0f));
    }
    else
    {
        _643 = cross(a, float3(1.0f, 0.0f, 0.0f));
    }
    float3 param = _643;
...
void selfTraj(float4 s, float4 e, float4 e2, float3 srcPos, float3 srcDir, float4 ev, float idx, float age, float life, float t, inout float3 pos, inout float3 vel)
{
    ...
    if (uVelMode == 4)
    {
        ...
        float param_11 = lu;
        float _2465 = selfSpeedAt(param_11);
        float ru = clamp(_2465 - lag, 0.0f, 1.0f);
        float param_12 = ru;
    ...
    {
        float param_18 = u;
        float _2631 = selfSpeedI(param_18);
        d = life * _2631;
        float param_19 = u;
        float _2636 = selfSpeedAt(param_19);
        scale = _2636;
    }
    else
    {
    ...
    if (uPlanarDrag > 0.0f)
    {
        ...
        float2 _2748 = (((dir * v0) * sp) + uWind).xz;
        vel = float3(_2748.x, vel.y, _2748.y);
    }
    ...
    if (uCurl > 0.0f)
    {
        ...
        float param_23 = u;
        float _2856 = curveE(param_23);
        pos += (((c_2 * uCurl) * _2856) * a);
    }
    if (uHasFloor == 1)
    {
...
void vert_main()
{
    ...
    float3 vel = param_16;
    vWp = pos;
    float k = (floor(uTime * uStripStep) + (aSeed.x * 97.3000030517578125f)) + (aIndex * 13.69999980926513671875f);
    float param_17 = k * 1.309999942779541015625f;
    float _3150 = stripHash(param_17);
    float L = lerp(uStripLen.x, uStripLen.y, _3150);
    float param_18 = (k * 3.769999980926513671875f) + 2.099999904632568359375f;
    float _3164 = stripHash(param_18);
    float W = lerp(uStripWide.x, uStripWide.y, _3164);
    float param_19 = (k * 5.5100002288818359375f) + 7.69999980926513671875f;
    float _3173 = stripHash(param_19);
    float wob = _3173;
    float param_20 = (k * 9.13000011444091796875f) + 3.2999999523162841796875f;
    float _3181 = stripHash(param_20);
    float yc = _3181 * uShapeRadius;
    float side = (aSeed.y > 0.5f) ? 1.0f : (-1.0f);
    float s = position.x;
    float v = position.y;
    ...
    float param_25 = u;
    float _3218 = curveA(param_25);
    float w = (((W * 0.5f) * safePow(param_21, param_22)) * safePow(param_23, param_24)) * _3218;
    float cy = ((uStripWave * 0.1599999964237213134765625f) * sin((s * 3.099999904632568359375f) + (wob * 6.283185482025146484375f))) * s;
    float4 mv = mul(float4(pos, 1.0f), modelViewMatrix);
    float3 param_26 = uAxis;
    float3 param_27 = float3(0.0f, 1.0f, 0.0f);
    float3 axisV = mul(float4(safeDir(param_26, param_27), 0.0f), modelViewMatrix).xyz;
    float2 _3264;
    if (length(axisV.xy) > 9.9999997473787516355514526367188e-06f)
    {
        _3264 = normalize(axisV.xy);
    }
    else
    {
        _3264 = float2(1.0f, 0.0f);
    }
    float2 along = _3264;
    float2 across = float2(-along.y, along.x);
    float2 _3298 = mv.xy + ((along * (s * L)) + (across * ((side * (yc + cy)) + (v * w))));
    mv = float4(_3298.x, _3298.y, mv.z, mv.w);
    gl_Position = mul(mv, projectionMatrix);
    vUv = float2(s, (v * 0.5f) + 0.5f);
    float idx = floor((aIndex * max(uStripCount - 1.0f, 1.0f)) + 0.5f);
    float _3323;
    if (uPalettes > 1.5f)
    {
        _3323 = mod(idx, 2.0f);
    ...
    vPalette = _3323;
    float param_28 = u;
    float _3332 = curveB(param_28);
    vAlpha = _3332 * smoothstep(0.0f, 0.02999999932944774627685546875f, age);
    if (uHasAlphaSpawn == 1)
    {
        float param_29 = aExtra.w;
        float _3346 = curveD(param_29);
        vAlpha *= _3346;
    }
    if (uTwinkleDepth > 0.0f)
    {
        float param_30 = abs(sin((uTime * uTwinkleFreq) + (aExtra2.y * 19.700000762939453125f)));
        float param_31 = 1.5f;
        vAlpha *= lerp(1.0f, safePow(param_30, param_31), uTwinkleDepth);
    }
}
...
void frag_main()
{
    ...
    float param = vPalette;
    float3 _119 = rampColor(param);
    float3 col = _119;
