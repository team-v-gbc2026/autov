...
- **Shaders:** trusted particle/surface GLSL is compiled offline through
...
- Only `particle` and `surface` shader revisions in `Shaders/layout.json`.
  Sub-particles, strips, trails and generator programs are rejected.
- PNG linear textures only. Procedural float/lattice textures are rejected.
...
- Double-sided, depth-tested, **non-depth-writing** draws only. Screen blend
  and other render-state variants fail import.
...
- Import is capped at 256 MiB / 4096 archive entries / 256 draws / 60,000
  particles per draw. Render data is copied/uploaded each update in this pass;
