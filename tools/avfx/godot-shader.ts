...
function stage(source: string, prefix: string, vertex: boolean) {
  ...
  ] : [{ name: "gl_FragColor", type: "vec4" }, { name: "gl_FragCoord", type: "vec4" }, { name: "gl_FrontFacing", type: "bool" }, { name: "viewMatrix", type: "mat4" }, { name: "inverseProjection", type: "mat4" }, { name: "screenUv", type: "vec2" }];
  ...
  const code = functions.map(f => {
    ...
    if (!vertex && f.name === "softDepth") body = `
...
export function godotShader(program: string, blend: string, depthTest = true, depthWrite = false, side = "double") {
  ...
  const declarations = [...uniforms.values()].map(b => `uniform ${b.type} ${b.name}${b.size ? `[${b.size}]` : ""}${b.type === "sampler2D" ? ` : ${b.name === "tDepth" ? "hint_depth_texture, filter_nearest" : b.name === "uSites" ? "filter_nearest" : "filter_linear_mipmap"}, ${b.name.toLowerCase().includes("noise") ? "repeat_enable" : "repeat_disable"}` : ""};`).join("\n");
  const swizzle = (type: string) => type === "float" ? ".x" : type === "vec2" ? ".xy" : type === "vec3" ? ".xyz" : "";
  return `shader_type spatial;
render_mode unshaded, ${side === "front" ? "cull_back" : side === "back" ? "cull_front" : "cull_disabled"}, fog_disabled, blend_premul_alpha, ${depthWrite ? "depth_draw_always" : "depth_draw_never"}${depthTest ? "" : ", depth_test_disabled"};
${declarations}
