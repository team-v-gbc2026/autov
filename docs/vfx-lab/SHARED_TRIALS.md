...
Shared trial documents and aggregate metadata live in the public `vfx-fixtures/presets` Supabase Storage prefix. These are draft experiments, not accepted production assets; scores refer to generation-time automated reviews. Playback uses the current renderer and can differ from the original review frames.
...
There is no checked-in local archive. Future refreshes are explicit: from `frontend`, run `node --import tsx scripts/publish-trial-presets.mjs --publish --ledger /path/to/the/existing/budget.json`. The script publishes only an allowlist of generated documents and aggregate metadata. It makes no paid requests.
