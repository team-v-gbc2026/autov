...
AUTOV_LOCAL_MODE=1 npm run dev -- --hostname 127.0.0.1 --port 3031
```

Open `http://127.0.0.1:3031/workspace`. Stop another local server on port 3031 first, or choose a free port. Each teammate's localhost URL refers to their own computer. A hosted branch preview uses the same `/workspace` path with the project's existing login configuration.
