...
This is a local-first generation integration: `/local` and `/workspace` under `npm run dev:local` use the integrated VFX screen. The authenticated Supabase project route preserves main's project/asset/draft workflow. Its generation backend is still not connected; local API keys are never enabled on the public deployment. Board image generation remains the upstream, unconnected UI. Board notes are not implicitly included in VFX prompts.
...
- Chrome: main Board displayed inside `/local`; timeline selection updates controls and chat target; From/To updates; real OpenAI edit for Fine white edge 0.80–1.10 s produced #ff8844, applied successfully, Undo/Redo exercised and the test edit undone. Local budget displayed about $4.46 including previous reservations.
