...
**Unreal:** the [experimental C++ importer/reference renderer](../../adapters/unreal/README.md) is integrated from Windows branch `feature/unreal-5.8-vfx-import` at `a40a64e`. It embeds AVFX dependencies in a cookable asset and evaluates particle/surface shaders over native indexed 3D geometry. It outputs to a dedicated reference-viewer target; it is not yet integrated with scene-world depth/lighting/Niagara. The Windows branch reports UE 5.8.2 build and three-angle comparison results (Fire 1.47-3.45/255; Shield 0.23-0.24/255). Mac verification covers source integration, not UE runtime execution. Packaged-game verification remains unproven.
...
Windows / Unreal 5.8 continuation: [self-contained Japanese handoff](WINDOWS-UE58-HANDOFF.ja.md). Unreal scene integration and packaged-game verification remain future work beyond the minimal reference importer. The real Studio component, mounted on its existing dev fixture route, successfully exported both Fire Projectile (8 draws) and Shield (7 draws) through Export 3D at 15 Hz. Authenticated project workflows were not part of this check.
